...
module Crystal
  ...
  def self.main(argc : Int32, argv : UInt8**) : Int32
    LibCrystalMain.__crystal_main(argc, argv); 0
  end
...
struct Descriptor
  ...
  def puts(msg : String) : Void
    LibC.fputs(msg, @fd)
  end

  def puts(msg) : Void
    LibC.fputs(msg.to_s, @fd)
  end
...
# class PrettyPrint
#   def initialize(@output : Descriptor, @maxwidth = 79, @newline = "\n", @indent = 0)
#     @output_width = 0
#     @buffer_width = 0
...
# macro ppp(obj, width = 80, newline = "\n", indent = 0)
#   printer = PrettyPrint.new(STDOUT, {{width}}, {{newline}}, {{indent}})
